...
# Aufruf:   docker buildx build --tag juergenzimmermann/angestellter:1.0.0-dockerfile .
...
# ---------------------------------------------------------------------------------------
FROM eclipse-temurin:19.0.1_10-jdk-jammy AS builder
...
RUN <<EOF
./gradlew bootJar
java -Djarmode=layertools -jar ./build/libs/angestellter-1.0.0.jar extract
EOF
